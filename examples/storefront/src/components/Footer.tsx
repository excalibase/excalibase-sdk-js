...
type WhoamiResponse = { shopifyWhoamiView: Array<{ role: string }> };
...
export function Footer({ identity }: { identity: DemoIdentity }) {
  ...
  return (
    <footer className="bg-white border-t border-slate-200 px-6 h-9 flex items-center gap-4 text-[11px] text-slate-500">
      ...
      <span>scope: <span className="font-mono text-slate-700">{identity.scope}</span></span>
      <span className="text-slate-200">|</span>
      <span className="flex items-center gap-1.5">
        Postgres role:
      ...
      <span className="font-mono">@excalibase/sdk · shopify schema</span>
    </footer>
  );
}
